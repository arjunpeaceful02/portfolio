...
var App = function() {
  ...
  if(document.querySelector(".template-blog-archive")) {
    var blog = new Blog();
  }
}
...
var Blog = function() {
  ...
  var initCache = function() {
    ...
    cache.bg_blue = document.querySelector('.banner-top.bg_dark-blue');
    cache.followerLight = cache.bg_blue.querySelector('.followerLight');
    cache.filters = cache.bg_blue.querySelectorAll('.filter-item');
    cache.filtersEl = cache.bg_blue.querySelectorAll('.filter-item .select-box li');
    cache.filterMobile = cache.bg_blue.querySelector('.filter-btn');
    cache.filterMobileClose = cache.bg_blue.querySelector('.close-btn');
    cache.cardsContainer = document.querySelector('.cards-list-flex');
    cache.noResult = cache.cardsContainer.querySelector('.no_result');
    cache.loadMore = document.querySelector('.load-more');
    cache.search =  document.querySelector('.search_filter');

    nbPosts = parseInt(cache.cardsContainer.dataset.post);
  };
  ...
  var registerEvents = function() {
    ...
    cache.filterMobile.addEventListener('click', openFiltersMobile);
    cache.filterMobileClose.addEventListener('click', closeFiltersMobile);

    for (var i = 0; i < cache.filters.length; i++) {
      cache.filters[i].addEventListener('click', toggleSelect);
    }
    for (var i = 0; i < cache.filtersEl.length; i++) {
      cache.filtersEl[i].addEventListener('click', clickFilter);
    }
    cache.loadMore.addEventListener('click', loadMore);
    cache.search.addEventListener('submit', search);
  };

  var hoverBlue = function() {
    darkBlueIn = true;
    cache.followerLight.style.display = 'block';
    cache.bg_blue.removeEventListener('mouseenter', hoverBlue);
    window.addEventListener('mousemove', cursor_light);
    anime({
      targets: cache.followerLight,
      opacity: [0, 0.7],
  ...
  var toggleSelect = function() {
    if(this.classList.contains('is-open')) {
      ...
      var box = this.querySelector('.select-box');
      anime({
        targets: box,
        height: 0,
        duration: 600,
      ...
      if(cache.bg_blue.querySelector('.filter-item.is-open')) {
        var openSelect = cache.bg_blue.querySelector('.filter-item.is-open');
        openSelect.classList.remove('is-open');
        var openSelectBox = openSelect.querySelector('.select-box');
        anime({
          targets: openSelectBox,
          height: 0,
          duration: 600,
          easing: 'easeOutExpo',
          complete: function() {
            openSelectBox.style.display = "none";
          }
        });
      }
  ...
  var clickFilter = function() {
    cache.search.querySelector('input').value = "";
    if(this.classList.contains('selected')) {
      if(this.closest('.filter-item').querySelector('li.selected')) {
        var subCatI = subCat.indexOf(this.closest('.filter-item').querySelector('li.selected').dataset.id);
        subCat.splice(subCatI, 1);
        this.closest('.filter-item').querySelector('li.selected').classList.remove('selected');
      }
      this.closest('.filter-item').querySelector('.txt').innerHTML = this.closest('.filter-item').querySelector('.txt').dataset.reset;
      var subCatI = subCat.indexOf(this.dataset.id);
      subCat.splice(subCatI, 1);
      ...
      this.closest('.filter-item').querySelector('.txt').innerHTML = this.innerHTML;
      this.classList.add('selected');
      subCat.push(this.dataset.id);
    }
    pOffset = 0;
    if(window.getComputedStyle(cache.loadMore).getPropertyValue("opacity") == 0) {
      console.log(window.getComputedStyle(cache.loadMore).getPropertyValue("opacity"))
      cache.loadMore.style.display = "inline-flex";
      anime({
        targets: cache.loadMore,
        opacity: 1,
        duration: 400,
        easing: 'easeOutSine',
      });
  ...
  var openFiltersMobile = function() {
    cache.body.classList.add('filter-is-open');
  }
  var closeFiltersMobile = function() {
    cache.body.classList.remove('filter-is-open');
  }
  ...
  var search = function() {
    ...
    var keyword = this.querySelector('input').value;

// RESET FILTERS
    subCat = [];
    if(cache.bg_blue.querySelector('li.selected')) {
      var selected = cache.bg_blue.querySelectorAll('li.selected');
      for (var i = 0; i < selected.length; i++) {
        selected[i].classList.remove('selected');
        selected[i].closest('.filter-item').querySelector('.txt').innerHTML = selected[i].closest('.filter-item').querySelector('.txt').dataset.reset;
      }
    }
    anime({
      targets: cache.loadMore,
    ...
    jQuery.ajax({
        ...
        success: function(data){
            ...
            var cards = cache.cardsContainer.querySelectorAll('.card');
            for (var i = 0; i < cards.length; i++) {
              cards[i].parentNode.removeChild(cards[i]);
            }
            if(data.length){
                for (var i = 0; i < data.length; i++) {
                  cache.cardsContainer.insertBefore(data[i],cache.cardsContainer.querySelector('.phantom-card'));
                }

                for (var i = 0; i < cache.filters.length; i++) {
                  cache.filters[i].style.pointerEvents = 'all';
                  anime({
  ...
  var load = function() {
    cache.noResult.style.display = "none";
    str = 'ppp=' + ppp + '&pOffset=' + pOffset + '&cats=' + subCat + '&action=filter_blog_ajax';
    ...
    jQuery.ajax({
        ...
        success: function(data){
            ...
            if(data.length){
                ...
                var nbPostsCats = parseInt(document.querySelector('.cards-list-flex .card').dataset.nbpost);
                if((pOffset+ppp) >= nbPostsCats) {
                  anime({
                    targets: cache.loadMore,
                    opacity: 0,
                    duration: 400,
...
var Nav = function() {
  ...
  var ajaxNav = function() {
    ...
    if(this.classList.contains('color_orange')) {
      cache.transition.classList.add('color_orange');
    }
            ...
            if(document.querySelector('.wpcf7')){
              ...
              anime({
                ...
                complete: function() {
                  ...
                  if(cache.transition.classList.contains('color_orange')) {
                    cache.transition.classList.remove('color_orange');
                  }
